...
extern "C" {
    ...
    /// * `attribute` - Target attribute to query (`CTRL_ATTR`)
    /// * `value` - The value of the attribute that will be populated upon function call
    fn XNVCTRLQueryStringAttribute(
        dpy: *const Display,
        screen: c_int,
        mask: c_uint,
        attribute: CTRL_ATTR,
        value: *const *mut c_char,
    ) -> c_int;
    ...
    /// * `attribute` - Attribute to query (`CTRL_ATTR`)
    /// * `value` - The value of the attribute that will be populated upon function call
    fn XNVCTRLQueryTargetStringAttribute(
        dpy: *const Display,
        target: CTRL_TARGET,
        id: c_int,
        mask: c_uint,
    ...
    fn XNVCTRLSetTargetAttributeAndGetStatus(
        dpy: *const Display,
        target: CTRL_TARGET,
        id: c_int,
        mask: c_uint,
    ...
    fn XNVCTRLQueryTargetBinaryData(
        dpy: *const Display,
        target: CTRL_TARGET,
        id: c_int,
        mask: c_uint,
...
impl NvidiaControl {
    ...
    pub fn init(lim: (u16, u16)) -> Result<NvidiaControl, String> {
        ...
        if dpy.is_null() {
            ...
            match unsafe { XNVCTRLQueryTargetCount(dpy, CTRL_TARGET::GPU, &mut gpu_count) } {
                XNV_OK => {}
                i => {
                    return Err(format!("XNVCtrl QueryCount(GPU) failed; error {}", i));
                }
            }
            ...
            for i in 0..gpu_count {
                ...
                match unsafe {
                    ...
                    XNV_OK => {
                        ...
                        let array: &[c_int] =
                            unsafe { slice::from_raw_parts(raw, 1usize + num_coolers) };
                    ...
                    i => {
                        return Err(format!(
                            "XNVCtrl BinaryData(COOLERS_USED_BY_GPU failed; {}",
                            i
                        ));
                    }
                }
...
impl NvFanController for NvidiaControl {
    ...
    fn get_ctrl_status(&self, gpu: u32) -> Result<NVCtrlFanControlState, String> {
        ...
        match unsafe {
            ...
            i => Err(format!(
                "XNVCtrl QueryAttr(COOLER_MANUAL_CONTROL) failed; error {}",
                i
            )),
        }
    }
    ...
    fn set_ctrl_type(&self, gpu: u32, typ: NVCtrlFanControlState) -> Result<(), String> {
        ...
        match unsafe {
            ...
            XNV_OK => Ok(()),
            i => Err(format!(
                "XNVCtrl SetAttr(COOLER_MANUAL_CONTROL) failed; error {}",
                i
            )),
        }
    }
    ...
    fn get_fanspeed(&self, _: u32, id: u32) -> Result<i32, String> {
        ...
        match unsafe {
            XNVCTRLQueryTargetAttribute(
                ...
                id as i32,
                0,
                CTRL_ATTR::THERMAL_COOLER_CURRENT_LEVEL,
                &mut tmp,
            )
        } {
            XNV_OK => Ok(tmp),
            i => Err(format!(
                "XNVCtrl QueryAttr(COOLER_CURRENT_LEVEL) failed; error {}",
                i
            )),
        }
    }
    ...
    fn get_fanspeed_rpm(&self, _: u32, id: u32) -> Result<i32, String> {
        ...
        match unsafe {
            XNVCTRLQueryTargetAttribute(
                ...
                id as i32,
                0,
                CTRL_ATTR::THERMAL_COOLER_SPEED,
                &mut tmp,
            )
        } {
            XNV_OK => Ok(tmp),
            i => Err(format!(
                "XNVCtrl QueryAttr(COOLER_SPEED) failed; error {}",
                i
            )),
        }
    }
    ...
    fn set_fanspeed(&self, _: u32, id: u32, speed: i32) -> Result<(), String> {
        ...
        match unsafe {
            ...
            XNV_OK => Ok(()),
            i => Err(format!(
                "XNVCtrl SetAttr(THERMAL_COOLER_LEVEL) failed; error {}",
                i
            )),
        }
    }
    ...
    fn get_version(&self) -> Result<String, String> {
        ...
        if nv_screen < 0 {
            return Err(
                "XNVCTRLIsNvScreen failed; no screens assigned to the NVidia driver".to_string(),
            );
        }
        ...
        match unsafe {
            XNVCTRLQueryStringAttribute(
                ...
                nv_screen,
                0,
                CTRL_ATTR::NVIDIA_DRIVER_VERSION,
                &v,
            )
        } {
            XNV_OK => {
                assert!(!v.is_null());
                Ok(unsafe {
                    CStr::from_ptr(v as *const c_char)
                        .to_str()
                        .unwrap()
                        .to_owned()
                })
            ...
            i => Err(format!(
                "XNVCtrl QueryAttr(NVIDIA_DRIVER_VERSION) failed; error {}",
                i
            )),
        }
    }
    ...
    fn get_adapter(&self, id: u32) -> Result<String, String> {
        ...
        match unsafe {
            ...
            i => Err(format!(
                "XNVCtrl QueryAttr(PRODUCT_NAME) failed; error {}",
                i
            )),
        }
    }
    ...
    fn get_utilization(&self, id: u32) -> Result<HashMap<&str, i32>, String> {
        ...
        match unsafe {
            ...
            i => Err(format!(
                "XNVCtrl QueryAttr(UTILIZATION) failed; error {}",
                i
            )),
        }
    }
